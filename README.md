...
Вам уже знакомо это приложение, вы работали с ним во время выполнения [домашнего задания по автотестам]
(https://github.com/dima117/testing-homework).
Теперь вам предстоит добавить в него CI/CD.

## 🚀 Демо

Приложение доступно по адресу: **http://158.160.188.157:3000/hw/store**

*Примечание: Сервер может быть недоступен, если он выключен или приложение не запущено*
...
- `REGISTRY_ID` - идентификатор Container Registry в Яндекс Облаке
- `YC_TOKEN` - IAM токен для доступа к Яндекс Облаку
- `SERVER_HOST` - IP-адрес виртуальной машины
- `SERVER_USER` - пользователь для SSH подключения к серверу

### Workflows

1. **PR Checks** - автоматически запускается при создании/изменении PR
   - Проверка линтера
   - Запуск тестов

2. **Create Release** - ручной запуск для создания релиза
...
3. **Fix Release** - ручной запуск для создания фикса
   - Принимает версию релиза на вход
   - Создание фикс-образа
   - Добавление комментария в Issue

4. **Deploy to Production** - ручной запуск для деплоя
   - Принимает версию релиза на вход
...
Приложение автоматически деплоится на сервер через GitHub Actions workflow "Deploy to Production".
